        ...
        const addSelectComponent = (obj) => {
            ...
            return newObj.map((component, index) => {
                if (component !== undefined) {
                    let className = `${component["settings" + viewport.type]?.className} ${(idUniqueIdentifier === component?.idUniqueIdentifier ? "  outline outline-offset-2 outline-cyan-400 z-[100]" : "")}`
                    ...
                    switch (component.group) {
                        case "element":
                            switch (component.type) {
                                case "text":
                                    return (
                                        <p onClick={ (event) => { onSelectItem(event, component) } }  id={component.idUniqueIdentifier + "preview"} className={className} key={component.idUniqueIdentifier}>
                                            {component.text}
                                        </p>
                                    )
                                case "image":
